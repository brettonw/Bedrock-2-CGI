...
from bedrock_cgi.constant import true, false, MIME_TYPE_JSON, CHARSET_UTF8
...
class Test (TestCase):
    def testBedrockCgi (self):
        ...
        os.environ[CONTENT_TYPE] = MIME_TYPE_JSON
        os.environ[CONTENT_LENGTH] = "{}".format (len (requestString))
        with patch("builtins.input", return_value=requestString + "\n"), patch("sys.stdout", new=StringIO()) as stdOut:
            ServiceBase.respond ()
